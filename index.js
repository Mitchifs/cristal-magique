		...
		if(/^.roue\s*pompes$/i.test(message.content)){
			...
			if(membres.some(m => m.user.id === message.author.id)){
				...
				if(/^.nord$/.test(message.content)){
					...
					if(joueurs[i].intelligence >= 1){
						connus += "Barque <= 4 Bûche + 2 Corde + 2 :zap:\n"
					}
					if(joueurs[i].intelligence >= 1){
						connus += "Plastique <= 1 Bouteille en plastique + *Feu*\n"
					}
					if(joueurs[i].intelligence >= 2){
						connus += "Feu de compétition <= 2 Bûche + *Feu*\n"
					}
					if(joueurs[i].intelligence >= 2){
						connus += "Lingot de fer <= 1 Fer brut + *Feu de compétition*\n"
					}
					if(joueurs[i].intelligence >= 2){
						connus += "Lingot d'or <= 1 Or brut + *Feu de compétition*\n"
					}
					if(joueurs[i].intelligence >= 2){
						connus += "Epée en fer <= 1 Lingot de fer + 1 Bâton + 1 Corde\n"
					}
